...
lists and vectors or that may result from computing the length of a
list or vector.  The \ide{length} procedure must return an exact
...
\label{sequencesection}

Moreso provides procedures which operate on a general \defining{sequence}
(either a well-formed list or a vector).

\begin{entry}{%
\proto{length}{ sequence}{library procedure}}

Returns the length of \var{sequence}.
...
\begin{entry}{%
\proto{element}{ sequence k}{procedure}}

\domain{\vr{k} must be a valid index of \var{sequence}.}
{\cf Element} returns element \vr{k} of \var{sequence}.

\begin{scheme}
(element '\#(1 1 2 3 5 8 13 21)
         5)  \lev  8
(element '(1 1 2 3 5 8 13 21)
...
\begin{entry}{%
\proto{set-element!}{ sequence k obj}{procedure}}

\domain{\vr{k} must be a valid index of \var{sequence}.}
{\cf Set-element!} stores \var{obj} in slot \vr{k} of \var{sequence}.
The value returned by {\cf set-element!}\ is unspecified.  % <!>

\begin{scheme}
(let ((vec (vector 0 '(2 2 2 2) "Anna")))
  (set-element! vec 1 '("Sue" "Sue"))
  vec)      \lev  \#(0 ("Sue" "Sue") "Anna")

(set-element! '\#(0 1 2) 1 "doe")  \lev  \scherror  ; constant vector%
\end{scheme}
\end{entry}
...
\begin{entry}{%
\proto{as-list}{ sequence \dotsfoo}{library procedure}}

Returns a newly allocated list of the objects contained in the given
sequences.

\begin{scheme}
(as-list '\#(dah dah didah) '(1))  \lev  (dah dah didah 1)
\end{scheme}
\end{entry}
...
to apply mutation procedures like \ide{set-element!} to strings returned
by this procedure.
...
Given a character, {\cf char\coerce{}integer} returns an exact integer
...
alphabetic, then the result of {\cf char-upcase} is upper case and the
result of {\cf char-downcase} is lower case.
...
\vest In phrases such as ``the elements of \var{vector} beginning with
...
subvector is referred to, and if \var{start} is zero and \var{end} is
the length of \var{vector}, then the entire vector is referred to.

Vectors are generally written using the notation
{\tt\#(\var{obj} \dotsfoo)}.
For example, a vector of length 3 containing the number zero in element
0, the list {\cf(2 2 2 2)} in element 1, and {\cf "Anna"} in element 2
...
\begin{entry}{%
\proto{as-vector}{ \var{sequence} \dotsfoo}{library procedure}}

Returns a newly allocated vector whose elements form the concatenation of the
given sequences.
...
\begin{entry}{%
\proto{subvector}{ vector start end}{library procedure}}

\domain{\var{Vector} must be a vector, and \var{start} and \var{end}
must be exact integers satisfying
$$0 \leq \var{start} \leq \var{end} \leq \hbox{\tt(length \var{vector})\rm.}$$}
{\cf Subvector} returns a newly allocated vector formed from the elements of
\var{vector} beginning with index \var{start} (inclusive) and ending with index
\var{end} (exclusive).
\end{entry}
...
Strings are vectors containing only characters.  
...
Returns \schtrue{} if \var{obj} is a vector in which every element is
a character, otherwise returns \schfalse.
\end{entry}
...
{\cf string=?}\ treats upper and lower case as distinct characters.
...
{\cf(as-list (list \vari{arg} \dotsfoo) \var{args})} as the actual
arguments.
