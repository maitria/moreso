...
(let ()
  ...
  (let-syntax ((defp
    ...
    (def assoc "assoc" 2)
    (def symbol? "symbol?" 1)
    (def symbol->char-vector "symbol->string" 1)
    (def char-vector->symbol "string->symbol" 1)
    (def number? "number?" 1)
    ...
    (def* number->char-vector "number->string" 1)
    (def* char-vector->number "string->number" 1)
    (def char? "char?" 1)
    (def char=? "char=?" 2)
    (def char>? "char>?" 2)
    (def char<? "char<?" 2)
    ...
    (def* (lambda args
	    (let* ((result-length (apply + (map
					     (lambda (s)
					       (if (vector? s)
						 (vector-length s)
						 (length s)))
					     args)))
		   (result (make-vector result-length))
		   (copy-vector (lambda (from to to-offset)
				  (let loop ((i 0))
				    (if (= i (vector-length from))
				      (+ to-offset i)
				      (begin
					(vector-set! to (+ i to-offset) (vector-ref from i))
					(loop (+ 1 i)))))))
		   (copy-list (lambda (from to to-offset)
				(let loop ((from from)
					   (to-offset to-offset))
				  (if (null? from)
				    to-offset
				    (begin
				      (vector-set! to to-offset (car from))
				      (loop (cdr from) (+ 1 to-offset))))))))
	      (let loop ((offset 0)
			 (args args))
		(if (not (= offset (vector-length result)))
		  (loop
		    (if (vector? (car args))
		      (copy-vector (car args) result offset)
		      (copy-list (car args) result offset))
		    (cdr args))))
	      result))
	  "as-vector" 0)
    ...
    (def (char-vector-compare >=) "string>=?" 2)
    (def (char-vector-compare <=) "string<=?" 2)
    (def (char-vector-compare-ci =) "string-ci=?" 2)
    (def (char-vector-compare-ci <) "string-ci<?" 2)
    (def (char-vector-compare-ci >) "string-ci>?" 2)
    (def (char-vector-compare-ci >=) "string-ci>=?" 2)
    (def (char-vector-compare-ci <=) "string-ci<=?" 2)
    ...
    (def (lambda (x) 
	   (%open-input-file (char-vector->string x)))
	 "open-input-file" 1)
    (def (lambda (x)
	   (%open-output-file (char-vector->string x)))
	 "open-output-file" 1)
    ...
    (def* (lambda arg
	    (if (null? arg)
	      (map string->char-vector (%command-line-arguments))
	      (%command-line-arguments (map char-vector->string (car arg)))))
	  "command-line-arguments" 0)
    (defp %error-port check-port "current-error-port")
    (def (lambda (x)
	   (%delete-file (char-vector->string x))
	   (%void))
	 "delete-file" 1)
    (def (lambda (fn)
	   (%file-exists? (char-vector->string fn)))
	 "file-exists?" 1)
    (def* (lambda arg
	    (if (null? arg)
	      (string->char-vector (%current-directory))
	      (%current-directory (char-vector->string (car arg)))))
	  "current-directory" 0)
    (def* %void "void" 0)
    (def* %exit "exit" 0)
    ...
    (def* (lambda (exp . env)
	  ...
	  "expand" 1)
    (def (lambda (cmd) 
	   (%system (char-vector->string cmd)))
	 "system" 1)
    (def %current-time "current-time" 0)
    (def (lambda (var)
	   (%get-environment-variable (char-vector->string var)))
    ...
    (def* (lambda (fn . args)
	  ...
	  "load-program" 1)
    ;;(defp *current-source-filename* check-char-vector "current-source-filename")
    (def* (lambda env
    ...
    (def* (lambda arg
	    (cond ((null? arg) *enable-call-trace*)
		  (else
		    (set! trace-buffer '())
		    (set! trace-buffer-end '())
		    (set! trace-buffer-length 0)
		    (set! *enable-call-trace* (car arg)))))
	  "enable-call-trace" 0)
    (def (lambda () *moreso-version*) "moreso-version" 0)
    (def* (lambda (msg . args)
	    (apply %error (char-vector->string msg) args))
	  "error" 1)
    (def* (lambda arg 
	    (if (null? arg)
	      %features
