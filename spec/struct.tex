...
\noindent\pproto{(element \var{sequence} \var{k})}{procedure}\unpenalty

indicates that the built-in procedure {\tt element} takes
two arguments, a vector or list \var{sequence} and an exact non-negative
integer \var{k} (see below).  The header lines
...
that an argument's name may indicate the argument's type.  For example, the
header line for {\tt element} given above dictates that the first argument
to {\tt element} must be a sequence.  The following naming conventions imply
type restrictions:
$$
\begin{tabular}{ll}
\var{obj}&any object\\
...
\var{sequence}&sequence (see section~\ref{sequencesection})\\
